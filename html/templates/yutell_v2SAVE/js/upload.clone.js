// upload.js
// Cloned from /js/upload.js
// EDITME: Check for updates in the original
/*
 * // EDITME @todo:
 * - redo HTML for 'listitem' (including class names)
 * - effects
 * - review 'errors' and 'successes' UX
 */
...
$(function () {
		...
		$.each(data.files, function (index, file) {
			...
			if (file_errors.length == 0) {
				...
				var file_node = '<li id="selected-file-' + index + '" >' +
							...
							'<div class="progress"><div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style=""><span class="progressvalue sr-only"></span></div></div>' +
	...
	.bind('fileuploadprogressall', function (e, data) {
		...
		$('#upload-video-selected-files-container').find('div.progress-bar').css('width', percentage +'%');
	...
	.bind('fileuploaddone', function (e, data) { 
		$.each(data.files, function (index, file) {
			...
			if (server_response.success == true && server_response.video_id >= 0) {
				...
				$('li#selected-file-' + index).html('<i class="fa fa-check-circle"></i> '+ pm_lang.swfupload_file + ': <em>' + file.name + '</em>');
