<form class="form-horizontal" name="register-form" id="register-form" method="post" action="{$form_action}" role="form">
	...
	<fieldset>
		...
		<div class="form-group">
			<label for="google_plus" class="col-md-2 control-label">{$lang.profile_social_google_plus|default:'Google+ URL'}</label>
			<div class="col-md-4"><input type="text" class="form-control" name="google_plus" {if isset($inputs.google_plus)}value="{$inputs.google_plus}"{else}value="{$userdata.social_links.google_plus}"{/if} placeholder="https://"></div>
		</div>
	</fieldset>
