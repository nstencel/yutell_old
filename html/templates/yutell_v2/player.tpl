...
{if $page == "detail"}
	{if $total_video_ads > 0}
		...
		<div id="embed_Playerholder" align="center">
		 <div id="{/literal}{$video_ad_hash}{literal}-playerholder" align="center">
		 </div>
		</div>
		<video src="" id="video-js" class="video-js vjs-default-skin" poster="" preload="auto" data-setup='{ "techOrder": ["flash","html5"], "controls": true, "autoplay": true }' width="640px" height="{/literal}{$smarty.const._PLAYER_H}{literal}" align="center"></video>
		<script type="text/javascript">
	...
	{elseif $video_data.video_player == "jwplayer"}
				...
				<script type='text/javascript'>
					var flashvars = {
						...
						'autostart': "{/literal}{$video_data.video_player_autoplay|default:$smarty.const._AUTOPLAY}{literal}", 
						{/literal}{/if}{literal}
						'enablejs': "true",
  ...
  {elseif $video_data.video_player == "jwplayer6"}
	...
	{literal}
		...
		{if $smarty.const._PM_VERSION >= '2.6.1'}
		...
		var video = videojs('video-js').ready(function(){
			...
			{if $video_data.source_id == $_sources.googledrive.source_id}
			{literal}
			player.src([{
				src: "{/literal}{$video_data.direct}{literal}",
				type: "video/mp4"
			...
			{/literal}{if $smarty.const._PM_VERSION >= '2.6.1'}{literal}
			player.socialShare({
				// twitter: {}
			});
			{/literal}{/if}{literal}
