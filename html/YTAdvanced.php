...
if(trim($_GET['keywords']) != '')
{
	...
	switch ($search_type)
	{
		...
		case 'video':
			...
			$sql = "SELECT SQL_CALC_FOUND_ROWS uniq_id, category, video_title, site_views, description, submitted, video_slug, yt_fun, yt_inte, yt_emot, yt_lang, featured, MATCH(video_title) AGAINST ('$searchstring') AS score 
					FROM pm_videos 
