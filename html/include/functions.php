...
function get_rated_video_list($limit = 0)
{
	global $config, $time_now_minute;
	...
	$result = mysql_query($sql);
	$row = mysql_fetch_assoc($result);
	$total_rated = (int) $row['total'];
	mysql_free_result($result);

	if ($total_rated > 0)
	{
		$rand_from = (($total_rated - $limit - 1) <= 0) ? 0 : abs(rand(0, $total_rated - $limit - 1));
		$sql = "SELECT uniq_id
				FROM pm_videos
				WHERE YT_Rated = '1' 
				  AND added <= $time_now_minute
	...
	{
		// no rated video assigned so pull a random one instead
		$rand_from = abs(rand(0, $config['published_videos'] - 1));
		$sql = "SELECT uniq_id
				FROM pm_videos
				WHERE added <= $time_now_minute
...
function insert_new_video($video_details, &$insert_id) // moved from /admin/functions.php since version 2.0
{
	...
	$sql = "INSERT INTO pm_videos (uniq_id, video_title, description, yt_id, yt_length, yt_thumb, category, submitted_user_id, submitted, added, url_flv, source_id, language, age_verification, yt_views, site_views, featured, restricted, allow_comments, allow_embedding, time, video_slug)
