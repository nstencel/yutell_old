...
if(trim($_GET['keywords']) != '')
{
	...
	switch ($search_type)
	{
		...
		case 'video':
			...
			// update hits for this search pharse table pm_searches - keeping track of search requests
			if ($page == 1)
			{
			...
			/*$sql = "SELECT SQL_CALC_FOUND_ROWS uniq_id, category, video_title, site_views, description, submitted, video_slug, yt_fun, yt_inte, yt_emot, yt_lang, MATCH(video_title) AGAINST ('$searchstring') AS score 
					FROM pm_videos 
			        WHERE added <= '". time() ."' AND MATCH(video_title) AGAINST ('$searchstring')
					ORDER BY score DESC 
					LIMIT ".$from.", ".$limit;*/
			// Search DB using Video Title Only index
                        $sql = "SELECT SQL_CALC_FOUND_ROWS id, MATCH(video_title) AGAINST ('$searchstring') AS score 
					FROM pm_videos 
			        WHERE added <= '". $time_now_minute ."' AND MATCH(video_title) AGAINST ('$searchstring')  OR ((video_title LIKE '%".$searchstring."%') OR (description LIKE '%".$searchstring."%') OR (yt_fun = '$searchstring') OR (yt_inte = '$searchstring') OR (yt_emot = '$searchstring') OR (yt_lang = '$searchstring'))
					ORDER BY score DESC 
					LIMIT ".$from.", ".$limit;
