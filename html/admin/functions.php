		...
		if($filter != '')
		{
			...
			switch($filter)
			{
				...
				case 'rated':
					...
					$sql .= " WHERE YT_Rated='0' ";
					
				break;

				case 'rating':
				
					$sql .= " WHERE YT_Rating='0' ";
		...
		while($r = mysql_fetch_array($query)) 
		{
			...
			if ($r['featured'] == '1')
			{
				$videos .= '<span class="label label-featured"><a href="videos.php?filter=featured" rel="tooltip" title="Click to list only featured videos">FEATURED</a></span> ';
                        ...
                        if ($r['rated'] == '1')
			{
				$videos .= '<span class="label label-rated"><a href="videos.php?filter=rated" rel="tooltip" title="Click to list only YT Rated videos">RATED</a></span> ';
			}
                        if ($r['rating'] == '1')
			{
				$videos .= '<span class="label label-rating"><a href="videos.php?filter=rating" rel="tooltip" title="Click to list only YT Rating videos">RATING</a></span> ';
			}
			if ($r['added'] > time())
			{
