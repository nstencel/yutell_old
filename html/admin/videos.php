...
$page	= (int) $_GET['page'];
$filter = 'added';
    $filters = array('broken', 'restricted', 'unchecked', 'localhost', 'featured', 'rated', 'rating', 'category', 'source', 'mostviewed', 'access', 'views', 'added', 'addedactive', 'scheduled', 'trash');
$filter_value = 'desc';
...
$total_rated_videos = count_entries('pm_videos', 'YT_Rated', 0);
$total_rating_videos = count_entries('pm_videos', 'YT_Rating', 0);
...
switch($filter)
{
	...
	break;
		...
		$total_videos = $total_rated_videos;
		
	break;
    
    	case 'rating':
		
		$total_videos = $total_rating_videos;
...
<p>The video status is represented by a round icon on the 4th column (from left). You can choose to check more than one video at a time by using the select all box (top left table corner) and then clicking the &quot;check status&quot; button.</p>
    ...
    <div class="entry-count">
        <ul class="pageControls">
            <li>
                <div class="floatL"><strong class="blue"><?php echo pm_number_format($total_videos); ?></strong><span>items</span></div>
...
if ($_POST['Submit'] == 'Restore selected')
{
	...
	if($total_ids > 0)
	{
		...
		foreach ($video_list_data as $k => $video)
		{
			...
			$sql = "INSERT INTO pm_videos (id, uniq_id, video_title, description, yt_id, yt_length, yt_thumb, category, submitted_user_id, submitted, added, url_flv, source_id, language, age_verification, yt_views, site_views, featured, YT_Rated, YT_Rating, restricted, allow_comments, allow_embedding, video_slug)
					VALUES ('". $video_id ."',
							'". $video['uniq_id'] ."', 
							'". secure_sql($video['video_title']) ."', 
							'". secure_sql($video['description']) ."', 
...
<div class="row-fluid">
	...
	<ul class="pm-inline-filters list-inline">
		...
		<li<?php if ($filter == 'rated') echo ' class="selected"'; ?>><a href="videos.php?filter=rated&page=1">YT To Be Rated <span class="count">(<?php echo pm_number_format($total_rated_videos); ?>)</span></a> <a href="videos.php" class="remove-filter">&times;</a> </li>
                <li<?php if ($filter == 'rating') echo ' class="selected"'; ?>><a href="videos.php?filter=rating&page=1">YT NOT Being Rated <span class="count">(<?php echo pm_number_format($total_rating_videos); ?>)</span></a> <a href="videos.php" class="remove-filter">&times;</a> </li>
          ...
          <div class="form-filter-inline">
			...
			<?php endif; ?>
              ...
              <select name="URL" onChange="window.parent.location=this.form.URL.options[this.form.URL.selectedIndex].value">
                <option value="videos.php?filter=rated&page=1" <?php if ($filter == 'rated') echo 'selected="selected"'; ?>>YuTell Rated</option>
                <option value="videos.php?filter=rating&page=1" <?php if ($filter == 'rating') echo 'selected="selected"'; ?>>YuTell Being Rated</option>
