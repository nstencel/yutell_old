...
if($_POST['submit'] != '')
{
	...
	$input['YT_Rating'] 		=	(int) $_POST['YT_Rating'];
	...
	// update site_views only if the input value has changed
        // YuTell Ratings area - MarkO Modified - to work when page submit button is pressed - CaribeSoft
        
    	if ($input['ytrating'] != $input['ytrating_input'])
	{
			
		$sql .= ", YT_Rating = '". $input['ytrating_input'] ."'";
	}

    	if ($input['ytrated'] != $input['ytrated_input'])
	{
			
		$sql .= ", YT_Rated = '". $input['ytrated_input'] ."'";
	}        
        
    	if ($input['yt_fun'] != $input['yt_fun_input'])
	{
		$input['yt_fun_input'] = abs( $input['yt_fun_input']); // positive values only		
		$sql .= ", yt_fun = '". $input['yt_fun_input'] ."'";
	}

    	if ($input['yt_inte'] != $input['yt_inte_input'])
	{
		$input['yt_inte_input'] = abs( $input['yt_inte_input']); // positive values only		
		$sql .= ", yt_inte = '". $input['yt_inte_input'] ."'";
	}        
        
    	if ($input['yt_lang'] != $input['yt_lang_input'])
	{
		$input['yt_lang_input'] =  $input['yt_lang_input']; // Accepts a Letter		
		$sql .= ", yt_lang = '". $input['yt_lang_input'] ."'";
	}

    	if ($input['yt_emot'] != $input['yt_emot_input'])
	{
		$input['yt_emot_input'] = abs( $input['yt_emot_input']); // positive values only		
		$sql .= ", yt_emot = '". $input['yt_emot_input'] ."'";
	}        
...
<form name="update" enctype="multipart/form-data" action="YTmodify.php?vid=<?php echo $uniq_id; ?>" method="post" onsubmit="return validateFormOnSubmit(this, 'Please fill in the required fields (highlighted)')">
<div class="container row-fluid" id="post-page">
	...
	<div class="span3">
		...
		<h4>Thumbnail</h4>
			...
			<?php else : ?>
			<img src="<?php echo make_url_https($r['yt_thumb']); ?>?cache_buster=<?php echo $time_now;?>" id="must" style="display:block;min-width:120px;width:100%;min-height:80px; no-repeat center center;" />
			<?php endif; ?>
			<div class="">
...
<!-- .widget -->
		...
		<h4>YuTell Details</h4>
			...
			<div class="control-group">
			<label>Being YuTell Rated? <span id="value-ytrating"><strong><?php echo ($r['YT_Rating'] == 1) ? 'yes' : 'no';?></strong></span> </label>
			<div class="controls" id="show-opt-ytrating">
				<label><input type="checkbox" name="ytrating_input" id="ytrating_input" value="1" <?php if($r['YT_Rating'] == 1) echo 'checked="checked"';?> /> Yes, YuTell Rating Happening</label>
			</div>
			</div> 
                        ...
                        <div class="control-group">
			<label class="control-label" for="">YuFunny Rating: <span id="value-yt-fun"><strong><?php echo $r['yt_fun'];?></strong></span> <a href="#" id="show-yt_fun"></a></label>
                        
			<input type="hidden" name="yt_fun" value="<?php echo $r['yt_fun'];?>" />
			<input type="text" name="yt_fun_input" id="yt_fun_input" value="<?php echo $r['yt_fun']; ?>" size="4" class="bigger span4" />
                        </div>

                         <div class="control-group">
			<label class="control-label" for="">YuInteresting  Rating: <span id="value-yt-int"><strong><?php echo $r['yt_inte'];?></strong></span> <a href="#" id="show-yt-inte"></a></label>
                        
			<input type="hidden" name="yt_inte" value="<?php echo $r['yt_inte'];?>" />
			<input type="text" name="yt_inte_input" id="yt_inte_input" value="<?php echo $r['yt_inte']; ?>" size="4" class="bigger span4" />
                        </div>
               
                        <div class="control-group">
			<label class="control-label" for="">YuEmotional  Rating: <span id="value-yt-emot"><strong><?php echo $r['yt_emot'];?></strong></span> <a href="#" id="show-yt-emot"></a></label>
                        
			<input type="hidden" name="yt_emot" value="<?php echo $r['yt_emot'];?>" />
			<input type="text" name="yt_emot_input" id="yt_emot_input" value="<?php echo $r['yt_emot']; ?>" size="4" class="bigger span4" />
                        </div>

                         <div class="control-group">
			<label class="control-label" for="">YuLanguage Rating: <span id="value-yt-lang"><strong><?php echo $r['yt_lang'];?></strong></span> <a href="#" id="show-yt-lang"></a></label>
                        
			<input type="hidden" name="yt_lang" value="<?php echo $r['yt_lang'];?>" />
			<input type="text" name="yt_lang_input" id="yt_lang_input" value="<?php echo $r['yt_lang']; ?>" size="4" class="bigger span4" />
                        </div>
               
                	<div class="control-group">
			<label>YuTell Rating Complete? <span id="value-ytrated"><strong><?php echo ($r['YT_Rated'] == 1) ? 'yes' : 'no';?></strong></span> </label>
			<div class="controls" id="show-opt-ytrating">
				<label><input type="checkbox" name="ytrated_input" id="ytrated_input" value="1" <?php if($r['YT_Rated'] == 1) echo 'checked="checked"';?> /> Yes, YuTell Rating Complete</label>
			</div>
			</div> 
			...
			<div class="control-group">
			<label class="control-label" for="">Comments: <span id="value-comments"><strong><?php echo ($r['allow_comments'] == 1) ? 'allowed' : 'closed';?></strong></span></label>
			...
			<div class="control-group">
			<label class="control-label" for="">Embedding: <span id="value-embedding"><strong><?php echo ($r['allow_embedding'] == 1) ? 'allowed' : 'disallowed';?></strong></span> </label>
			...
			<div class="control-group">
			<label>Featured: <span id="value-featured"><strong><?php echo ($r['featured'] == 1) ? 'yes' : 'no';?></strong></span> </label>
			...
			<div class="control-group">
			<label class="control-label" for="">Requires registration: <span id="value-register"><strong><?php echo ($r['restricted'] == 1) ? 'yes' : 'no';?></strong></span> </label>
		...
		<label class="checkbox inline"><input type="radio" name="restricted" id="restricted" value="0" <?php if ($r['restricted'] == 0) echo 'checked="checked"'; ?> /> No</label>
			...
			<div class="control-group">
			<label class="control-label" for="">Views: <span id="value-views"><strong><?php echo $r['site_views'];?></strong></span> </label>
			...
			<div class="control-group">
			<label class="control-label" for="">Submitted by: <span id="value-submitted"><strong><?php echo htmlspecialchars($r['submitted']); ?></strong></span> </label>
			...
			<div class="control-group">
			<label class="control-label" for="">Publish date: <span id="value-publish"><strong><?php echo date("M d, Y g:i A", $r['added']);?></strong></span> </label>
