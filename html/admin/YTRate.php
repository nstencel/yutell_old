...
{
	echo pm_alert_success('Video moved to Trash. <a href="YTmodify.php?vid='. $_GET['vid'] .'&a=4&page='. $page .'&filter='. $filter .'&fv='. $filter_value .'">Undo</a>');
}
...
else if ($_POST['Submit'] == 'Trash selected')
{
	...
	if($total_ids > 0)
	{
		...
		echo pm_alert_success('Videos removed successfully. You can restore them from the <a href="YTRate.php?filter=trash&page=1">Trash</a>.');
	}
	else
	{
		echo pm_alert_warning('Please select something first.');
...
//			exit();
		...
		{
			echo '<meta http-equiv="refresh" content="0;URL=YTRate.php" />';
			exit();
		}
	}
	else
	{
		...
		?>
		<form name="delete" method="post" action="YTRate.php?action=9">
...
<h4>SEARCH RESULTS FOR "<em><?php echo $_POST['keywords']; ?></em>" <a href="#" onClick="parent.location='YTRate.php'" class="opac5"><i class="icon-remove-sign"></i></a></h4>
<?php endif; ?>
</div>
...
<div class="row-fluid">
	...
	<div class="span4">
	    ...
	    <form name="videos_per_page" action="YTRate.php" method="get" class="form-inline pull-right">
    ...
    <div class="row-fluid">
        ...
        <div class="btn-group input-prepend">
          <div class="form-filter-inline">
			...
			<button type="button" id="appendedInputButtons" class="btn btn-danger btn-strong" onClick="parent.location='YTRate.php'">Remove filter</button>
			<?php else : ?>
			<button type="button" id="appendedInputButtons" class="btn">Filter</button>
			<?php endif; ?>
              <?php if ( ! $in_trash) : ?>
			  <form name="other_filter" action="YTRate.php" class="form-inline">
              <select name="URL" onChange="window.parent.location=this.form.URL.options[this.form.URL.selectedIndex].value">
                <option value="YTRate.php">by videos ...</option>
                <option value="YTRate.php?filter=mostviewed&page=1" <?php if ($filter == 'mostviewed') echo 'selected="selected"'; ?>>Most viewed</option>
                <option value="YTRate.php?filter=added&page=1" <?php if ($filter == 'added') echo 'selected="selected"'; ?> >Added date</option>
                <option value="YTRate.php?filter=addedactive&page=1" <?php if ($filter == 'addedactive') echo 'selected="selected"'; ?> >Added date (excl. scheduled)</option>
                <option value="YTRate.php?filter=scheduled&page=1" <?php if ($filter == 'scheduled') echo 'selected="selected"'; ?> >Scheduled only</option>
              </select>
              </form>

              <form name="category_filter" action="YTRate.php" method="get" class="form-inline">
        ...
        <div class="pull-right">
        	<?php if ( ! $in_trash) : ?>
            <form name="search" action="YTRate.php" method="post" class="form-search-listing form-inline">
...
<div class="clearfix"></div>
<form name="videos_checkboxes" id="videos_checkboxes" action="YTRate.php?page=<?php echo $page;?>&filter=<?php echo $filter;?>&fv=<?php echo $filter_value;?>" method="post">
 <table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered pm-tables tablesorter">
  <thead>
   <tr>
    ...
    <th width="90">
    	<?php if ( ! $in_trash) : ?>
		<a href="YTRate.php?filter=added&fv=<?php echo ($filter_value == 'desc' && $filter == 'added') ? 'asc' : 'desc';?>" rel="tooltip" title="Sort <?php echo ($filter_value == 'desc' && $filter == 'added') ? 'ascending' : 'descending';?>">Added</a>
		<?php else : ?>
		Added
		<?php endif; ?>
	</th>
	<th width="65">
		<?php if ( ! $in_trash) : ?>
		<a href="YTRate.php?filter=views&fv=<?php echo ($filter_value == 'desc' && $filter == 'views') ? 'asc' : 'desc';?>" rel="tooltip" title="Sort <?php echo ($filter_value == 'desc' && $filter == 'views') ? 'ascending' : 'descending';?>">Views</a></th>
		<?php else : ?>
		Views
		<?php endif; ?>
