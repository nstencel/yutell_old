...
switch($action)
{
	...
	case 'approve':
		...
		chdir ('/');
		$video_time = shell_exec('ffmpeg -i var/www/html/uploads/videos/' . $video_details['url_flv'] . ' 2>&1 | grep Duration | sed \'s/..Duration:.\(.\{8\}\).*$/\1/\'');
		
		if (strcmp(substr($video_time, 0, 2), "00") == 0) { // Compare first two characters of string to 00 to see if it's hours or minutes long
			$video_time = substr($video_time, 3);
		}
		
		$video_details['time'] = $video_time;
